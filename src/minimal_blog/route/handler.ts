...
import { PostDataInterface, ResponseConfig } from "../utils/interfaces";
import formidable from "formidable";
import { firestore, storage } from "../utils/config";
...
BlogRouter.post(
  ...
  async (req: Request, res: Response<ResponseConfig>) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Post sent for processing", status: 200 });
...
async function formParsing(req: Request) {
